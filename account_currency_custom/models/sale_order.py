...
class SaleOrder(models.Model):
    ...
    def _apply_total_currency_label(self, view_arch, view_type="form"):
        ...
        for node in doc.xpath("//field[@name='amount_total_curr'][@widget='monetary']"):
            node.set(
                "string",
    ...
    amount_total_curr = fields.Monetary(
        ...
        help="Total amount converted into the selected company’s currency.",
    ...
    @api.depends("amount_total", "currency_id", "total_company_id", "date_order")
    def _compute_amount_company(self):
        for order in self:
            if not order.total_company_id or not order.total_company_id.currency_id:
                order.amount_total_curr = 0.0
                continue

            order.amount_total_curr = order.currency_id._convert(
                order.amount_total,
                order.total_company_id.currency_id,
                order.total_company_id,
    ...
    def _get_under_validation_exceptions(self):
        ...
        res.extend(["total_company_id", "amount_total_curr"])
        return res
