...
class WebsiteSaleVariantControllerEx(WebsiteSaleVariantController):
    ...
    def get_combination_info_website(self, product_template_id, product_id, combination, add_qty, **kw):
        combination = super(WebsiteSaleVariantControllerEx, self).get_combination_info_website(product_template_id,
    ...
    def shop_payment_confirmation(self, **post):
        ...
        if sale_order_id:
            ...
            if order_id.partner_id.last_limit_line_update_order_id and order_id.partner_id.last_limit_line_update_order_id.id == order_id.id:
                update_ok = False
            limit_qty_product_line_ids = order_id.order_line.filtered(
                lambda line: line.product_id.get_variant_qty_limit(cart_qty=line.product_uom_qty).get("apply_qty_limit")
            )
            if limit_qty_product_line_ids and update_ok:
                if order_id.partner_id.product_limit_line_ids:
                    ...
                    for limit_qty_product_line_id in limit_qty_product_line_ids:
                        rule = limit_qty_product_line_id.product_id.get_variant_qty_limit(
                            cart_qty=limit_qty_product_line_id.product_uom_qty)
                        remove_after_days = rule.get("remove_customer_limit_after", 0)
                        ...
                        if product_limit_line:
                            ...
                            limit_lines.append((0, 0, {
                                ...
                                "limit_end_date": fields.Datetime().now().date() + timedelta(days=remove_after_days)
                            }))
                    if limit_lines:
